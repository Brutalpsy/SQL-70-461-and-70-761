...
--COMBINES THE SHAPE INTO ONE
declare @i as geometry
select @i = geometry::UnionAggregate(GXY) 
from tblGeom

Select @i as CombinedShapes
--COLLECTION COMBINES THEM AS SEPARETE SHAPES, AS A COLLECTION OF SHAPES
...
Select @i as CombinedShapes
--union all
select geometry::EnvelopeAggregate(GXY) as Envelope from tblGeom -- ENVELOPE USES STRAIGHT LINES
--union all
select geometry::ConvexHullAggregate(GXY) as Envelope from tblGeom --SMOTHES IT OUT
